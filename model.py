...
class DownsamplerBlock(nn.Cell):
    def __init__(self, in_feature_num, out_feature_num, weight_init):
        ...
        self.conv = nn.Conv2d(in_feature_num, out_feature_num-in_feature_num, \
            3, stride=2, padding=(1, 1, 1, 1), has_bias=True, weight_init=weight_init, pad_mode="pad")
    ...
    def construct(self, x):
        a = self.conv(x)
        b = self.pool(x)
        # ops.Concat反向传播的时候用了F.tuple_len, 不支持列表, sbms, 而且动态图模式下能运行, 静态图模式下运行不了
        # output = self.cat([a, b]) 
        output = self.cat((a, b))
...
class non_bottleneck_1d(nn.Cell):
    def __init__(self, chann, dropprob, dilated, weight_init):
        ...
        self.conv3x1_1 = nn.Conv2d(chann, chann, (3, 1), stride=1, padding=(1, 1, 0, 0),\
            pad_mode='pad', has_bias=True, weight_init=weight_init)
        self.conv1x3_1 = nn.Conv2d(chann, chann, (1, 3), stride=1, padding=(0, 0, 1, 1),\
            pad_mode='pad', has_bias=True, weight_init=weight_init)
        
        self.conv3x1_2 = nn.Conv2d(chann, chann, (3, 1), stride=1, padding=(dilated, dilated, 0, 0),\
            pad_mode='pad', has_bias=True, dilation=(dilated, 1), weight_init=weight_init)
        self.conv1x3_2 = nn.Conv2d(chann, chann, (1, 3), stride=1, padding=(0, 0, dilated, dilated),\
            pad_mode='pad', has_bias=True, dilation=(1, dilated), weight_init=weight_init)
...
class Encoder(nn.Cell):
    def __init__(self, weight_init, train=True):
        super(Encoder, self).__init__()

        if train:
            # # # # # # # # # # # # # 
            drop_prob = [0.03, 0.3] # 1,2,4阶段使用
            # drop_prob = [0.03, 0.2] # 3阶段使用
            # # # # # # # # # # # # # 
        else:
            drop_prob = [.0, .0]
...
class Encoder_pred(nn.Cell):
    def __init__(self, num_class, weight_init, train=True):
        ...
        self.encoder = Encoder(weight_init, train)
...
class ERFNet(nn.Cell):
    def __init__(self, num_class, init_conv, train=True):
        super(ERFNet, self).__init__()
        self.encoder = Encoder(init_conv, train)
